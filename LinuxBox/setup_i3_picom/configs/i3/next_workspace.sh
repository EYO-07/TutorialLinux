...
#-> $next || next is initially the current worskpace number plus one from current output
#-> $next | & | % if next is in valid range || goto the workspace assigned by next 
#-> $next | & || % next not exists on all outputs || break ~ this is our target workspace
#-> $next | & || %% next exists and is on current output || break ~ this is our target workspace
#-> $next | & || %%% else || increment the next workspace number
#.
#1. A || B : means that B belongs to structure of A or a subproblem of A 
#2. $next : construction of next workspace 
...
while true; do
    ...
    ws=$(jq -c --argjson n "$next" '.[] | select(.num == $n)' <<< "$current_json")
